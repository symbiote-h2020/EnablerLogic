...
public class RabbitManager {
	...
	// public void sendPlaceholderMessage(String placeholder) { // arg should be
	// object instead of String, e.g. Resource
	// Gson gson = new Gson();
	// String message = gson.toJson(placeholder);
	// sendMessage(this.placeholderExchangeName, this.placeholderRoutingKey,
	// message);
	...
	private void sendMessage(String exchange, String routingKey, String message) {
		...
		try {
			channel = this.connection.createChannel();
			props = new AMQP.BasicProperties().builder().contentType("application/json").build();
